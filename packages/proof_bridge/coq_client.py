"""Client helper for the Coq proof bridge."""
...
def _default_runner(
    ...
    return obligations.ProofResult(
        ...
        diagnostics="Coq prover not configured",
    )


class CoqClient:
    """Minimal Coq client suitable for dependency injection in tests."""

    def __init__(self, *, runner: Runner | None = None, timeout: float | None = 30.0) -> None:
...
def prove(
    ...
    *,
    ...
    client = CoqClient(runner=runner, timeout=timeout)
...
def _normalise_result(
    ...
    if not isinstance(response, dict):
        raise TypeError("Coq runner must return ProofResult or mapping")
    data = dict(response)
